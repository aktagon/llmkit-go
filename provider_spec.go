// Code generated by codegen/generate.py; DO NOT EDIT.

package llmkit

// providerSpec is HOW the library talks to a provider (PRIVATE): the
// internal wire/transform spec (response paths, auth scheme, role mappings,
// ...) consumed only by the runtime. Volatile; never the public surface.
type providerSpec struct {
	Name                    string
	BaseURL                 string
	Endpoint                string
	DefaultModel            string
	EnvVar                  string
	DefaultMaxTokens        int
	ResponseTextPath        string
	AuthScheme              string
	AuthHeader              string
	AuthPrefix              string
	AuthQueryParam          string
	RequiredHeader          string
	RequiredHeaderValue     string
	SystemPlacement         string
	RoleMappings            map[string]string
	UsageInputPath          string
	UsageOutputPath         string
	ReasoningTokensPath     string // empty when provider does not report reasoning tokens separately
	FinishReasonPath        string // empty when provider does not surface a finish reason
	FinishMessagePath       string // empty when provider does not surface a free-text finish message
	StreamFinishReasonPath  string // event_name:json.path or bare json.path; empty disables stream-time capture (ADR-013)
	StreamFinishMessagePath string // same form as StreamFinishReasonPath; empty for every provider today (ADR-013)
	WrapsOptionsIn          string
	SafetySettingsWirePath  string // top-level field name for safetySettings; empty = not supported
	ModelInBody             bool
	ErrorMessagePath        string
	ErrorTypePath           string
	AccessKeyEnvVar         string
	SecretKeyEnvVar         string
	SessionTokenEnvVar      string
	RegionEnvVar            string
	ServiceName             string
}

// providerSpecs returns the internal provider spec registry. The map is
// built once (package-level var) and shared; callers treat it as read-only.
func providerSpecs() map[string]providerSpec { return providerSpecRegistry }

var providerSpecRegistry = map[string]providerSpec{
	"ai21": {
		Name:                "ai21",
		BaseURL:             "https://api.ai21.com",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "jamba-1.5-large",
		EnvVar:              "AI21_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"anthropic": {
		Name:                "anthropic",
		BaseURL:             "https://api.anthropic.com",
		Endpoint:            "/v1/messages",
		DefaultModel:        "claude-sonnet-4-6",
		EnvVar:              "ANTHROPIC_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "content[0].text",
		AuthScheme:          "HeaderAPIKey",
		AuthHeader:          "x-api-key",
		AuthPrefix:          "",
		AuthQueryParam:      "",
		RequiredHeader:      "anthropic-version",
		RequiredHeaderValue: "2023-06-01",
		SystemPlacement:     "TopLevelField",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"user":      "user",
		},
		UsageInputPath:          "usage.input_tokens",
		UsageOutputPath:         "usage.output_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "stop_reason",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "message_stop:stop_reason",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"assemblyai": {
		Name:                "assemblyai",
		BaseURL:             "https://api.assemblyai.com",
		Endpoint:            "/v2/transcript",
		DefaultModel:        "best",
		EnvVar:              "ASSEMBLYAI_API_KEY",
		DefaultMaxTokens:    0,
		ResponseTextPath:    "",
		AuthScheme:          "HeaderAPIKey",
		AuthHeader:          "authorization",
		AuthPrefix:          "",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"user": "user",
		},
		UsageInputPath:          "",
		UsageOutputPath:         "",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             false,
		ErrorMessagePath:        "error",
		ErrorTypePath:           "status",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"azure": {
		Name:                "azure",
		BaseURL:             "https://REPLACE-WITH-YOUR-RESOURCE.openai.azure.com",
		Endpoint:            "/openai/deployments/{model}/chat/completions?api-version=2024-10-21",
		DefaultModel:        "gpt-4o",
		EnvVar:              "AZURE_OPENAI_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "HeaderAPIKey",
		AuthHeader:          "api-key",
		AuthPrefix:          "",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             false,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"bedrock": {
		Name:                "bedrock",
		BaseURL:             "https://bedrock-runtime.{region}.amazonaws.com",
		Endpoint:            "/model/{model}/converse",
		DefaultModel:        "anthropic.claude-sonnet-4-20250514-v1:0",
		EnvVar:              "AWS_ACCESS_KEY_ID",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "output.message.content[0].text",
		AuthScheme:          "SigV4",
		AuthHeader:          "",
		AuthPrefix:          "",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "TopLevelField",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"user":      "user",
		},
		UsageInputPath:          "usage.inputTokens",
		UsageOutputPath:         "usage.outputTokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "inferenceConfig",
		SafetySettingsWirePath:  "",
		ModelInBody:             false,
		ErrorMessagePath:        "message",
		ErrorTypePath:           "",
		AccessKeyEnvVar:         "AWS_ACCESS_KEY_ID",
		SecretKeyEnvVar:         "AWS_SECRET_ACCESS_KEY",
		SessionTokenEnvVar:      "AWS_SESSION_TOKEN",
		RegionEnvVar:            "AWS_REGION",
		ServiceName:             "bedrock",
	},
	"cerebras": {
		Name:                "cerebras",
		BaseURL:             "https://api.cerebras.ai",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "llama-3.3-70b",
		EnvVar:              "CEREBRAS_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"cohere": {
		Name:                "cohere",
		BaseURL:             "https://api.cohere.com/compatibility",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "command-r-plus",
		EnvVar:              "COHERE_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"deepseek": {
		Name:                "deepseek",
		BaseURL:             "https://api.deepseek.com",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "deepseek-chat",
		EnvVar:              "DEEPSEEK_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"doubao": {
		Name:                "doubao",
		BaseURL:             "https://ark.cn-beijing.volces.com/api/v3",
		Endpoint:            "/chat/completions",
		DefaultModel:        "doubao-1.5-pro-32k-250115",
		EnvVar:              "ARK_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"ernie": {
		Name:                "ernie",
		BaseURL:             "https://qianfan.baidubce.com/v2",
		Endpoint:            "/chat/completions",
		DefaultModel:        "ernie-4.0-8k",
		EnvVar:              "QIANFAN_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"fireworks": {
		Name:                "fireworks",
		BaseURL:             "https://api.fireworks.ai/inference",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "accounts/fireworks/models/llama-v3p3-70b-instruct",
		EnvVar:              "FIREWORKS_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"google": {
		Name:                "google",
		BaseURL:             "https://generativelanguage.googleapis.com",
		Endpoint:            "/v1beta/models/{model}:generateContent",
		DefaultModel:        "gemini-2.5-flash",
		EnvVar:              "GOOGLE_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "candidates[0].content.parts[0].text",
		AuthScheme:          "QueryParamKey",
		AuthHeader:          "",
		AuthPrefix:          "",
		AuthQueryParam:      "key",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "SiblingObject",
		RoleMappings: map[string]string{
			"assistant": "model",
			"user":      "user",
		},
		UsageInputPath:          "usageMetadata.promptTokenCount",
		UsageOutputPath:         "usageMetadata.candidatesTokenCount",
		ReasoningTokensPath:     "usageMetadata.thoughtsTokenCount",
		FinishReasonPath:        "candidates[0].finishReason",
		FinishMessagePath:       "candidates[0].finishMessage",
		StreamFinishReasonPath:  "candidates[0].finishReason",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "generationConfig",
		SafetySettingsWirePath:  "safetySettings",
		ModelInBody:             false,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.status",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"grok": {
		Name:                "grok",
		BaseURL:             "https://api.x.ai",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "grok-3-fast",
		EnvVar:              "XAI_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "choices[0].finish_reason",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "choices[0].finish_reason",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"groq": {
		Name:                "groq",
		BaseURL:             "https://api.groq.com/openai",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "llama-3.3-70b-versatile",
		EnvVar:              "GROQ_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"inworld": {
		Name:                "inworld",
		BaseURL:             "https://api.inworld.ai",
		Endpoint:            "/tts/v1/voice",
		DefaultModel:        "inworld-tts-2",
		EnvVar:              "INWORLD_API_KEY",
		DefaultMaxTokens:    0,
		ResponseTextPath:    "",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Basic",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"user": "user",
		},
		UsageInputPath:          "",
		UsageOutputPath:         "",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "message",
		ErrorTypePath:           "code",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"jan": {
		Name:                "jan",
		BaseURL:             "http://localhost:1337",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "",
		EnvVar:              "JAN_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"llamacpp": {
		Name:                "llamacpp",
		BaseURL:             "http://localhost:8080",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "",
		EnvVar:              "LLAMACPP_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"lmstudio": {
		Name:                "lmstudio",
		BaseURL:             "http://localhost:1234",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "",
		EnvVar:              "LM_STUDIO_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"minimax": {
		Name:                "minimax",
		BaseURL:             "https://api.minimax.chat",
		Endpoint:            "/v1/text/chatcompletion_v2",
		DefaultModel:        "MiniMax-Text-01",
		EnvVar:              "MINIMAX_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "base_resp.status_msg",
		ErrorTypePath:           "base_resp.status_code",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"mistral": {
		Name:                "mistral",
		BaseURL:             "https://api.mistral.ai",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "mistral-large-latest",
		EnvVar:              "MISTRAL_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"moonshot": {
		Name:                "moonshot",
		BaseURL:             "https://api.moonshot.ai",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "moonshot-v1-8k",
		EnvVar:              "MOONSHOT_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"ollama": {
		Name:                "ollama",
		BaseURL:             "http://localhost:11434",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "",
		EnvVar:              "OLLAMA_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"openai": {
		Name:                "openai",
		BaseURL:             "https://api.openai.com",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "gpt-4o-2024-08-06",
		EnvVar:              "OPENAI_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "usage.completion_tokens_details.reasoning_tokens",
		FinishReasonPath:        "choices[0].finish_reason",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "choices[0].finish_reason",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"openrouter": {
		Name:                "openrouter",
		BaseURL:             "https://openrouter.ai/api",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "openai/gpt-4o",
		EnvVar:              "OPENROUTER_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"perplexity": {
		Name:                "perplexity",
		BaseURL:             "https://api.perplexity.ai",
		Endpoint:            "/chat/completions",
		DefaultModel:        "sonar-pro",
		EnvVar:              "PERPLEXITY_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"pixverse": {
		Name:                "pixverse",
		BaseURL:             "https://app-api.pixverse.ai",
		Endpoint:            "/openapi/v2/video/text/generate",
		DefaultModel:        "v4.5",
		EnvVar:              "PIXVERSE_API_KEY",
		DefaultMaxTokens:    0,
		ResponseTextPath:    "",
		AuthScheme:          "HeaderAPIKey",
		AuthHeader:          "API-KEY",
		AuthPrefix:          "",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"user": "user",
		},
		UsageInputPath:          "",
		UsageOutputPath:         "",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "ErrMsg",
		ErrorTypePath:           "ErrCode",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"qwen": {
		Name:                "qwen",
		BaseURL:             "https://dashscope-intl.aliyuncs.com/compatible-mode",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "qwen-plus",
		EnvVar:              "DASHSCOPE_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"recraft": {
		Name:                "recraft",
		BaseURL:             "https://external.api.recraft.ai",
		Endpoint:            "/v1/images/generations",
		DefaultModel:        "recraftv3",
		EnvVar:              "RECRAFT_API_TOKEN",
		DefaultMaxTokens:    0,
		ResponseTextPath:    "",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"user": "user",
		},
		UsageInputPath:          "",
		UsageOutputPath:         "",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "message",
		ErrorTypePath:           "code",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"sambanova": {
		Name:                "sambanova",
		BaseURL:             "https://api.sambanova.ai",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "Meta-Llama-3.3-70B-Instruct",
		EnvVar:              "SAMBANOVA_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"together": {
		Name:                "together",
		BaseURL:             "https://api.together.xyz",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "meta-llama/Llama-3.3-70B-Instruct-Turbo",
		EnvVar:              "TOGETHER_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"vertex": {
		Name:                "vertex",
		BaseURL:             "https://{location}-aiplatform.googleapis.com/v1/projects/{project_id}/locations/{location}/publishers/google/models",
		Endpoint:            "/{model}:predict",
		DefaultModel:        "imagen-3.0-generate-002",
		EnvVar:              "VERTEX_BEARER_TOKEN",
		DefaultMaxTokens:    0,
		ResponseTextPath:    "",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"user": "user",
		},
		UsageInputPath:          "",
		UsageOutputPath:         "",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "predictions[0].raiFilteredReason",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             false,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.status",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"vidu": {
		Name:                "vidu",
		BaseURL:             "https://api.vidu.com",
		Endpoint:            "/ent/v2/text2video",
		DefaultModel:        "viduq3-pro",
		EnvVar:              "VIDU_API_KEY",
		DefaultMaxTokens:    0,
		ResponseTextPath:    "",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Token",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"user": "user",
		},
		UsageInputPath:          "",
		UsageOutputPath:         "",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "message",
		ErrorTypePath:           "code",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"vllm": {
		Name:                "vllm",
		BaseURL:             "http://localhost:8000",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "",
		EnvVar:              "VLLM_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"workersai": {
		Name:                "workersai",
		BaseURL:             "https://api.cloudflare.com/client/v4/accounts/{account_id}/ai/v1",
		Endpoint:            "/chat/completions",
		DefaultModel:        "@cf/meta/llama-3.1-8b-instruct",
		EnvVar:              "CLOUDFLARE_API_TOKEN",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "choices[0].finish_reason",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "choices[0].finish_reason",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"yi": {
		Name:                "yi",
		BaseURL:             "https://api.01.ai",
		Endpoint:            "/v1/chat/completions",
		DefaultModel:        "yi-large",
		EnvVar:              "YI_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
	"zhipu": {
		Name:                "zhipu",
		BaseURL:             "https://open.bigmodel.cn/api/paas",
		Endpoint:            "/v4/chat/completions",
		DefaultModel:        "glm-4-plus",
		EnvVar:              "ZHIPU_API_KEY",
		DefaultMaxTokens:    4096,
		ResponseTextPath:    "choices[0].message.content",
		AuthScheme:          "BearerToken",
		AuthHeader:          "Authorization",
		AuthPrefix:          "Bearer",
		AuthQueryParam:      "",
		RequiredHeader:      "",
		RequiredHeaderValue: "",
		SystemPlacement:     "MessageInArray",
		RoleMappings: map[string]string{
			"assistant": "assistant",
			"system":    "system",
			"tool":      "tool",
			"user":      "user",
		},
		UsageInputPath:          "usage.prompt_tokens",
		UsageOutputPath:         "usage.completion_tokens",
		ReasoningTokensPath:     "",
		FinishReasonPath:        "",
		FinishMessagePath:       "",
		StreamFinishReasonPath:  "",
		StreamFinishMessagePath: "",
		WrapsOptionsIn:          "",
		SafetySettingsWirePath:  "",
		ModelInBody:             true,
		ErrorMessagePath:        "error.message",
		ErrorTypePath:           "error.type",
		AccessKeyEnvVar:         "",
		SecretKeyEnvVar:         "",
		SessionTokenEnvVar:      "",
		RegionEnvVar:            "",
		ServiceName:             "",
	},
}
